...
## Decision Documentation Rule (MANDATORY)

Every non-trivial choice must leave a written trail. Code minimization wins on LOC; this rule wins on rationale. They do not conflict — keep the code minimal, keep the *reasoning* documented elsewhere. The masterdoc is `docs/ARCHITECTURE.md`. The ledger of individual decisions lives in `docs/adr/NNNN-slug.md`. Read `docs/adr/README.md` before starting work on a new area so you don't re-litigate settled questions.

### When you MUST write an ADR (`docs/adr/NNNN-slug.md`)
- Choosing between two viable approaches and rejecting one.
- Introducing a new abstraction, module, protocol, file, or public type.
- Picking a library, API, or platform behavior (e.g. MSSession, UIImageAsset, xcstrings vs .strings).
- Defining or changing a public interface — function signature, URL query param, MSMessage layout field, encoded move format.
- UX patterns the user can perceive — button placement, send flow, animation timing, ring colors, what a button actually does on tap, copy wording, localization choice.
- Anything you decide while working autonomously that was not explicitly approved in the prompt.
- Any workaround, invariant, or ordering constraint that will look wrong to a future reader without the context (e.g. the `confirmMove()` reorder, the hidden sizing-reference ZStack).

### When you can skip
- Pure renames, formatting, comment edits, dead-code deletions.
- Bug fixes where there is only one correct answer.
- Implementing a spec that already exists in `docs/ARCHITECTURE.md`, an ADR, or `FEATURE_ROADMAP.md`.

### ADR format (keep it short — 10–40 lines)
```
# NNNN — <slug>

- **Status**: Accepted | Superseded by ADR-XXXX | Deprecated
- **Date**: YYYY-MM-DD
- **Version**: v1.X if tied to a release
- **Source**: commit hash / file:line / roadmap section / chat

## Context
What forced the decision. The problem, the constraints, the forcing function.

## Decision
What we chose, stated as a single sentence of fact.

## Alternatives considered
What we rejected, and why. One line each is fine.

## Consequences
What this commits us to. What it makes harder later. Any invariants future code must preserve.
```

Number ADRs sequentially. Never edit an accepted ADR in place — supersede it with a new one that links back.

### LLM-specific addendum
When working autonomously on a multi-step task, surface design decisions to the user **as you make them**, not after. If you abstract three call sites into a helper, pick a coordinate system, choose an animation curve, invent a naming convention, or decide how to split a new module — say so in chat AND write an ADR in the same turn. The user has explicitly said they do not want to discover architectural decisions by reading the diff. "Silent abstraction" is a bug, not a feature.

Before starting a non-trivial task, check `docs/adr/` for decisions that touch the area you are changing. If a new decision supersedes an old ADR, write the new ADR with a `Supersedes: ADR-NNNN` header and mark the old one `Status: Superseded by ADR-NNNN`.
